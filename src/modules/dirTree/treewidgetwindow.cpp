...
// Search stuff
void TreeWidgetWindow::updateViewWithFilter(const QString& filter)
{
    ...
    rootItem->setText(0, QDir(m_lastOpenedDir).dirName());
    ...
    if (filter == "File Names") {
        ...
        scanDirectory(rootItem, m_lastOpenedDir);
    } else {
        // Grouped metadata view
        QMap<QString, QList<AudioMetadata>> groupedItems;

        // Collect and group all audio files
        std::function<void(const QString&)> collectAndGroup = [&](const QString &path) {
            QDir dir(path);
            ...
            const QFileInfoList dirs = dir.entryInfoList(QDir::Dirs | QDir::NoDotAndDotDot);
            for (const QFileInfo &dirInfo : dirs) {
                collectAndGroup(dirInfo.absoluteFilePath());
            }
        };

        collectAndGroup(m_lastOpenedDir);
